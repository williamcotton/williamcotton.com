...
const defaultNodeRenderers = {
  [BLOCKS.PARAGRAPH]: (node, next, index) =>
    h('p', { key: `p-${index}` }, next(node.content)),
  [BLOCKS.HEADING_1]: (node, next, index) =>
    h('h1', { key: `h1-${index}` }, next(node.content)),
  [BLOCKS.HEADING_2]: (node, next, index) =>
    h('h2', { key: `h2-${index}` }, next(node.content)),
  [BLOCKS.HEADING_3]: (node, next, index) =>
    h('h3', { key: `h3-${index}` }, next(node.content)),
  [BLOCKS.HEADING_4]: (node, next, index) =>
    h('h4', { key: `h4-${index}` }, next(node.content)),
  [BLOCKS.HEADING_5]: (node, next, index) =>
    h('h5', { key: `h5-${index}` }, next(node.content)),
  [BLOCKS.HEADING_6]: (node, next, index) =>
    h('h6', { key: `h6-${index}` }, next(node.content)),
  ...
  [BLOCKS.EMBEDDED_ASSET]: (node, next, index) => {
    ...
    return h(
      'img',
      { src: url, alt: title, key: `img-${index}` },
      next(node.content)
    );
  },
  [BLOCKS.UL_LIST]: (node, next, index) =>
    h('ul', { key: `ul-${index}` }, next(node.content)),
  [BLOCKS.OL_LIST]: (node, next, index) =>
    h('ol', { key: `ol-${index}` }, next(node.content)),
  [BLOCKS.LIST_ITEM]: (node, next, index) =>
    h('li', { key: `li-${index}` }, next(node.content)),
